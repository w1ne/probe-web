...
impl<'probe> Armv8a<'probe> {
    pub(crate) async fn new(
        ...
        if !state.initialized() {
            ...
            let edscr = Edscr(memory.read_word_32(address).await?);
    ...
    async fn execute_instruction(&mut self, instruction: u32) -> Result<Edscr, Error> {
        ...
        self.memory
            .write_word_32(address, final_instruction)
            .await?;

        // Wait for completion
        let address = Edscr::get_mmio_address_from_base(self.base_address)?;
        let mut edscr = Edscr(self.memory.read_word_32(address).await?);

        while !edscr.ite() {
            edscr = Edscr(self.memory.read_word_32(address).await?);
        }
        ...
        if edscr.err() || edscr.a() {
            ...
            self.memory.write_word_32(address, edrcr.into()).await?;
    ...
    async fn execute_instruction_with_result_32(&mut self, instruction: u32) -> Result<u32, Error> {
        // Run instruction
        let mut edscr = self.execute_instruction(instruction).await?;
    ...
    /// Execute an instruction on the CPU and return the result
    async fn execute_instruction_with_result_64(&mut self, instruction: u32) -> Result<u64, Error> {
        // Run instruction
        let mut edscr = self.execute_instruction(instruction).await?;
        ...
        let mut result: u64 = (self.memory.read_word_32(address).await? as u64) << 32;

        let address = Dbgdtrtx::get_mmio_address_from_base(self.base_address)?;
        result |= self.memory.read_word_32(address).await? as u64;

        Ok(result)
    }

    async fn execute_instruction_with_input_32(
        &mut self,
        instruction: u32,
        value: u32,
    ) -> Result<(), Error> {
    ...
    async fn execute_instruction_with_input_64(
        &mut self,
        instruction: u32,
        value: u64,
    ) -> Result<(), Error> {
        ...
        let address = Dbgdtrtx::get_mmio_address_from_base(self.base_address)?;
        self.memory.write_word_32(address, high_word).await?;

        let address = Dbgdtrrx::get_mmio_address_from_base(self.base_address)?;
        self.memory.write_word_32(address, low_word).await?;
    ...
    async fn writeback_registers_aarch32(&mut self) -> Result<(), Error> {
    ...
    async fn writeback_registers_aarch64(&mut self) -> Result<(), Error> {
        ...
        for i in writeback_iter {
            if let Some((val, writeback)) = self.state.register_cache[i as usize] {
                if writeback {
                    match i {
                        0..=30 => {
                            self.set_reg_value(i, val.try_into()?).await?;
                        }
                        31 => {
                            // Move val to r0
                            self.set_reg_value(0, val.try_into()?).await?;
                        ...
                        34..=65 => {
                            ...
                            self.set_reg_value(0, val as u64).await?;
                            ...
                            self.set_reg_value(0, (val >> 64) as u64).await?;
    ...
    async fn writeback_registers(&mut self) -> Result<(), Error> {
        if self.state.is_64_bit {
            self.writeback_registers_aarch64().await?;
        } else {
            self.writeback_registers_aarch32().await?;
        }
    ...
    async fn prepare_for_clobber(&mut self, reg: u16) -> Result<(), Error> {
        if self.state.register_cache[reg as usize].is_none() {
            // cache reg since we're going to clobber it
            let val = self.read_core_reg(RegisterId(reg)).await?;
    ...
    async fn set_reg_value(&mut self, reg: u16, value: u64) -> Result<(), Error> {
        if self.state.is_64_bit {
            // MRS DBGDTR_EL0, X<n>
    ...
    async fn ack_cti_halt(&mut self) -> Result<(), Error> {
        let mut ack = CtiIntack(0);
        ack.set_ack(0, 1);

        let address = CtiIntack::get_mmio_address_from_base(self.cti_address)?;
        self.memory.write_word_32(address, ack.into()).await?;

        loop {
            let address = CtiTrigoutstatus::get_mmio_address_from_base(self.cti_address)?;
            let trig_status = CtiTrigoutstatus(self.memory.read_word_32(address).await?);
    ...
    async fn read_core_reg_32(&mut self, reg_num: u16) -> Result<RegisterValue, Error> {
        ...
        match reg_num {
            0..=14 => {
                ...
                let reg_value = self.execute_instruction_with_result_32(instruction).await?;
            ...
            15 => {
                ...
                let pc = self.execute_instruction_with_result_32(instruction).await?;
            ...
            16 => {
                ...
                // Read from r0
                let instruction = build_mcr(14, 0, 0, 0, 5, 0);
                let cpsr = self.execute_instruction_with_result_32(instruction).await?;
            ...
            17..=48 => {
                ...
                // Read from r0
                let instruction = build_mcr(14, 0, 0, 0, 5, 0);
                let mut value = self.execute_instruction_with_result_32(instruction).await? as u64;

                // Read from r1
                let instruction = build_mcr(14, 0, 1, 0, 5, 0);
                value |= (self.execute_instruction_with_result_32(instruction).await? as u64) << 32;
            ...
            49 => {
                ...
                // Read from r0
                let instruction = build_mcr(14, 0, 0, 0, 5, 0);
                let value = self.execute_instruction_with_result_32(instruction).await?;
    ...
    async fn read_core_reg_64(&mut self, reg_num: u16) -> Result<RegisterValue, Error> {
        match reg_num {
            0..=30 => {
                // GP register
                ...
                let reg_value = self.execute_instruction_with_result_64(instruction).await?;
            ...
            31 => {
                ...
                let pc = self.execute_instruction_with_result_64(instruction).await?;
            ...
            32 => {
                ...
                // Read from x0
                let instruction = aarch64::build_msr(2, 3, 0, 4, 0, 0);
                let sp = self.execute_instruction_with_result_64(instruction).await?;
            ...
            33 => {
                ...
                // Read from x0
                let instruction = aarch64::build_msr(2, 3, 0, 4, 0, 0);
                let psr: u32 = self.execute_instruction_with_result_64(instruction).await? as u32;
            ...
            34..=65 => {
                ...
                let instruction = aarch64::build_msr(2, 3, 0, 4, 0, 0);
                let mut value: u128 =
                    self.execute_instruction_with_result_64(instruction).await? as u128;
                ...
                let instruction = aarch64::build_msr(2, 3, 0, 4, 0, 0);
                value |=
                    (self.execute_instruction_with_result_64(instruction).await? as u128) << 64;
            ...
            66 => {
                ...
                // Read from x0
                let instruction = aarch64::build_msr(2, 3, 0, 4, 0, 0);
                let fpsr: u32 = self.execute_instruction_with_result_64(instruction).await? as u32;
    ...
    where
        F: AsyncFnOnce(&mut Self) -> Result<R, Error>,
    {
        // save halt status
        ...
        F: AsyncFnOnce(&mut Self) -> Result<R, Error>,
    {
        ...
        self.set_memory_access_mode(false).await?;
    ...
    async fn read_cpu_memory_aarch32_32(&mut self, address: u64) -> Result<u32, Error> {
        ...
        self.with_core_halted(async |armv8a| {
            ...
            armv8a.set_reg_value(0, address.into()).await?;

            // Read data to r1 - LDR r1, [r0], #4
            let instruction = build_ldr(1, 0, 4);

            armv8a.execute_instruction(instruction).await?;

            // Move from r1 to transfer buffer - MCR p14, 0, r1, c0, c5, 0
            let instruction = build_mcr(14, 0, 1, 0, 5, 0);
            armv8a.execute_instruction_with_result_32(instruction).await
        })
        .await
    }

    async fn read_cpu_memory_aarch64_bytes(
        &mut self,
        address: u64,
        data: &mut [u8],
    ) -> Result<(), Error> {
        self.with_core_halted(async |armv8a| {
            ...
            armv8a.set_reg_value(0, address).await?;
            ...
            for d in data {
                ...
                *d = armv8a
                    .execute_instruction_with_result_32(instruction)
                    .await? as u8;
            }
    ...
    async fn read_cpu_memory_aarch64_64(&mut self, address: u64) -> Result<u64, Error> {
        self.with_core_halted(async |armv8a| {
            ...
            armv8a.execute_instruction_with_result_64(instruction).await
        })
        .await
    }
    ...
    async fn write_cpu_memory_aarch32_32(&mut self, address: u64, data: u32) -> Result<(), Error> {
        ...
        self.with_core_halted(async |armv8a| {
            ...
            armv8a.set_reg_value(0, address.into()).await?;
            armv8a.set_reg_value(1, data.into()).await?;
    ...
    async fn write_cpu_memory_aarch64_bytes(
        &mut self,
        address: u64,
        data: &[u8],
    ) -> Result<(), Error> {
        self.with_core_halted(async |armv8a| {
            ...
            for d in data {
                armv8a.set_reg_value(1, u64::from(*d)).await?;
    ...
    async fn write_cpu_memory_aarch64_64(&mut self, address: u64, data: u64) -> Result<(), Error> {
        self.with_core_halted(async |armv8a| {
            ...
            // Load r0 with the address to write to
            armv8a.set_reg_value(0, address).await?;
            armv8a.set_reg_value(1, data).await?;
    ...
    async fn set_memory_access_mode(&mut self, enable_ma_mode: bool) -> Result<(), Error> {
        let address = Edscr::get_mmio_address_from_base(self.base_address)?;
        let mut edscr: Edscr = Edscr(self.memory.read_word_32(address).await?);
        edscr.set_ma(enable_ma_mode);
        self.memory.write_word_32(address, edscr.into()).await?;

        Ok(())
    }

    async fn write_cpu_memory_aarch64_fast(
        &mut self,
        address: u64,
        data: &[u8],
    ) -> Result<(), Error> {
        self.with_core_halted(async |armv8a| {
            ...
            if !prefix.is_empty() {
                armv8a
                    .write_cpu_memory_aarch64_bytes(address, prefix)
                    .await?;
            ...
            // write aligned part
            armv8a
                .write_cpu_memory_aarch64_fast_inner(address, aligned)
                .await?;
            ...
            if !suffix.is_empty() {
                armv8a
                    .write_cpu_memory_aarch64_bytes(address, suffix)
                    .await?;
            }
            Ok(())
        })
    ...
    async fn write_cpu_memory_aarch64_fast_inner(
        &mut self,
        address: u64,
        data: &[u8],
    ) -> Result<(), Error> {
        ...
        self.with_memory_access_mode(async |armv8a| {
            for d in data.chunks(4) {
                ...
                armv8a.memory.write_word_32(dbgdtr_rx_address, word).await?;
            }
            Ok(())
        })
        .await?;
        ...
        if Edscr(self.memory.read_word_32(edscr_address).await?).err() {
            // under-run or abort
            ...
            edrcr.set_cse(true);
            self.memory
                .write_word_32(edrcr_address, edrcr.into())
                .await?;
    ...
    async fn read_cpu_memory_aarch64_fast(
        &mut self,
        address: u64,
        data: &mut [u8],
    ) -> Result<(), Error> {
        self.with_core_halted(async |armv8a| {
            ...
            if !prefix.is_empty() {
                armv8a
                    .read_cpu_memory_aarch64_bytes(address, prefix)
                    .await?;
                address += u64::try_from(prefix.len()).unwrap();
            }
            ...
            // read aligned part
            armv8a
                .read_cpu_memory_aarch64_fast_inner(address, aligned)
                .await?;
            address += u64::try_from(aligned.len()).unwrap();
            ...
            if !suffix.is_empty() {
                armv8a
                    .read_cpu_memory_aarch64_bytes(address, suffix)
                    .await?;
            }
    ...
    async fn read_cpu_memory_aarch64_fast_inner(
        &mut self,
        address: u64,
        data: &mut [u8],
    ) -> Result<(), Error> {
        ...
        self.memory
            .write_word_32(editr_address, msr_instruction)
            .await?;

        // wait for TXfull == 1
        let edscr_address = Edscr::get_mmio_address_from_base(self.base_address)?;
        while !{ Edscr(self.memory.read_word_32(edscr_address).await?) }.txfull() {}
        ...
        self.with_memory_access_mode(async |armv8a| {
            ...
            for d in data.chunks_mut(4) {
                ...
                let tmp = armv8a
                    .memory
                    .read_word_32(dbgdtr_tx_address)
                    .await?
                    .to_le_bytes();
        ...
        // error check
        let address = Edscr::get_mmio_address_from_base(self.base_address)?;
        let edscr = Edscr(self.memory.read_word_32(address).await?);
        if edscr.err() {
            // clear error flag
            let edrcr_address = Edrcr::get_mmio_address_from_base(self.base_address)?;
            let mut edrcr = Edrcr(0);
...
impl CoreInterface for Armv8a<'_> {
    ...
    async fn halt(&mut self, timeout: Duration) -> Result<CoreInformation, Error> {
        if !matches!(self.state.current_state, CoreStatus::Halted(_)) {
            ...
            self.memory.write_word_32(address, cti_gate.into()).await?;
            ...
            self.memory.write_word_32(address, pulse.into()).await?;
    ...
    async fn run(&mut self) -> Result<(), Error> {
        ...
        loop {
            let edprsr = Edprsr(self.memory.read_word_32(address).await?);
            if edprsr.sdr() {
                break;
            }
        }
    ...
    async fn step(&mut self) -> Result<CoreInformation, Error> {
        ...
        let mut edecr = Edecr(self.memory.read_word_32(edecr_address).await?);

        edecr.set_ss(true);
        self.memory
            .write_word_32(edecr_address, edecr.into())
            .await?;
    ...
    async fn read_core_reg(&mut self, address: RegisterId) -> Result<RegisterValue, Error> {
        ...
        let result = if self.state.is_64_bit {
            self.read_core_reg_64(reg_num).await
        } else {
            self.read_core_reg_32(reg_num).await
        };
    ...
    async fn available_breakpoint_units(&mut self) -> Result<u32, Error> {
        if self.num_breakpoints.is_none() {
            ...
            let eddfr = Eddfr(self.memory.read_word_32(address).await?);
    ...
    async fn hw_breakpoints(&mut self) -> Result<Vec<Option<u64>>, Error> {
        ...
        for bp_unit_index in 0..num_hw_breakpoints {
            ...
            let mut bp_value = self.memory.read_word_32(bp_value_addr).await? as u64;
            bp_value |= (self.memory.read_word_32(bp_value_addr + 4).await? as u64) << 32;
    ...
    async fn set_hw_breakpoint(&mut self, bp_unit_index: usize, addr: u64) -> Result<(), Error> {
        ...
        self.memory.write_word_32(bp_value_addr, addr_low).await?;
        self.memory
            .write_word_32(bp_value_addr + 4, addr_high)
            .await?;
        self.memory
    ...
    async fn clear_hw_breakpoint(&mut self, bp_unit_index: usize) -> Result<(), Error> {
        ...
        self.memory.write_word_32(bp_value_addr + 4, 0).await?;
    ...
    async fn reset_catch_set(&mut self) -> Result<(), Error> {
        self.sequence
            .reset_catch_set(&mut *self.memory, CoreType::Armv8a, Some(self.base_address))
            .await?;
    ...
    async fn reset_catch_clear(&mut self) -> Result<(), Error> {
        self.sequence
            .reset_catch_clear(&mut *self.memory, CoreType::Armv8a, Some(self.base_address))
            .await?;
...
impl MemoryInterface for Armv8a<'_> {
    ...
    async fn read_word_64(&mut self, address: u64) -> Result<u64, Error> {
        if self.state.is_64_bit {
            self.read_cpu_memory_aarch64_64(address).await
        } else {
            let mut ret = self.read_cpu_memory_aarch32_32(address).await? as u64;
            ret |= (self.read_cpu_memory_aarch32_32(address + 4).await? as u64) << 32;
    ...
    async fn read_word_32(&mut self, address: u64) -> Result<u32, Error> {
        if self.state.is_64_bit {
            self.read_cpu_memory_aarch64_32(address).await
        } else {
            self.read_cpu_memory_aarch32_32(address).await
        }
    }
    ...
    async fn read_64(&mut self, address: u64, data: &mut [u64]) -> Result<(), Error> {
        if self.state.is_64_bit {
            ...
            self.read_cpu_memory_aarch64_fast(address, data).await?;
        } else {
    ...
    async fn write_word_64(&mut self, address: u64, data: u64) -> Result<(), Error> {
        if self.state.is_64_bit {
            self.write_cpu_memory_aarch64_64(address, data).await
        } else {
            let low_word = data as u32;
            let high_word = (data >> 32) as u32;

            self.write_cpu_memory_aarch32_32(address, low_word).await?;
    ...
    async fn write_word_32(&mut self, address: u64, data: u32) -> Result<(), Error> {
        if self.state.is_64_bit {
            self.write_cpu_memory_aarch64_32(address, data).await
        } else {
            self.write_cpu_memory_aarch32_32(address, data).await
        }
    }
    ...
    async fn write_64(&mut self, address: u64, data: &[u64]) -> Result<(), Error> {
        if self.state.is_64_bit {
            ...
            self.write_cpu_memory_aarch64_fast(address, data).await?;
        } else {
...
mod test {
    ...
    async fn armv8a_core_halted() {
        ...
        assert!(!armv8a.core_halted().await.unwrap());
        assert!(armv8a.core_halted().await.unwrap());
    }

    #[pollster::test]
    async fn armv8a_wait_for_core_halted() {
        let mut probe = MockProbe::new(false);
        let mut state = CortexAState::new();
    ...
    #[pollster::test]
    async fn armv8a_status_running() {
        let mut probe = MockProbe::new(false);
        let mut state = CortexAState::new();
        ...
        assert_eq!(CoreStatus::Running, armv8a.status().await.unwrap());
    }

    #[pollster::test]
    async fn armv8a_status_halted() {
        let mut probe = MockProbe::new(false);
        let mut state = CortexAState::new();
        ...
        assert_eq!(
            ...
            armv8a.status().await.unwrap()
        );
    }

    #[pollster::test]
    async fn armv8a_read_core_reg_common() {
        ...
        assert_eq!(
            ...
            armv8a.read_core_reg(RegisterId(2)).await.unwrap()
        );
    ...
    #[pollster::test]
    async fn armv8a_read_core_reg_common_64() {
    ...
    #[pollster::test]
    async fn armv8a_read_core_reg_pc() {
        const REG_VALUE: u32 = 0xABCD;
        ...
        assert_eq!(
            RegisterValue::from(REG_VALUE),
            armv8a.read_core_reg(RegisterId(15)).await.unwrap()
        );
    ...
    #[pollster::test]
    async fn armv8a_read_core_64_reg_pc() {
        const REG_VALUE: u64 = 0xFFFF_EEEE_0000_ABCD;
        ...
        assert_eq!(
            RegisterValue::from(REG_VALUE),
            armv8a.read_core_reg(RegisterId(32)).await.unwrap()
        );
    ...
    #[pollster::test]
    async fn armv8a_read_core_reg_cpsr() {
        const REG_VALUE: u32 = 0xABCD;
        ...
        assert_eq!(
            RegisterValue::from(REG_VALUE),
            armv8a.read_core_reg(RegisterId(16)).await.unwrap()
        );
    ...
    #[pollster::test]
    async fn armv8a_read_core_64_reg_cpsr() {
        const REG_VALUE: u32 = 0xABCD;
        ...
        assert_eq!(
            RegisterValue::from(REG_VALUE),
            armv8a.read_core_reg(RegisterId(33)).await.unwrap()
        );
    ...
    async fn armv8a_halt() {
        ...
        assert_eq!(
            REG_VALUE as u64,
            armv8a.halt(Duration::from_millis(100)).await.unwrap().pc
        );
    }
    ...
    async fn armv8a_run() {
        ...
        armv8a.run().await.unwrap();
    }

    #[pollster::test]
    async fn armv8a_available_breakpoint_units() {
        ...
        assert_eq!(BP_COUNT, armv8a.available_breakpoint_units().await.unwrap());
    }

    #[pollster::test]
    async fn armv8a_hw_breakpoints() {
        const BP_COUNT: u32 = 4;
        ...
        let results = armv8a.hw_breakpoints().await.unwrap();
    ...
    #[pollster::test]
    async fn armv8a_set_hw_breakpoint() {
        ...
        armv8a.set_hw_breakpoint(0, BP_VALUE).await.unwrap();
    }

    #[pollster::test]
    async fn armv8a_clear_hw_breakpoint() {
        let mut probe = MockProbe::new(false);
        let mut state = CortexAState::new();
        ...
        armv8a.clear_hw_breakpoint(0).await.unwrap();
    }

    #[pollster::test]
    async fn armv8a_read_word_32() {
        ...
        assert_eq!(
            MEMORY_VALUE,
            armv8a.read_word_32(MEMORY_ADDRESS).await.unwrap()
        );
    }

    #[pollster::test]
    async fn armv8a_read_word_32_aarch64() {
        const MEMORY_VALUE: u32 = 0xBA5EBA11;
        const MEMORY_ADDRESS: u64 = 0x12345678;
    ...
    async fn armv8a_read_word_8() {
        ...
        assert_eq!(0xBA, armv8a.read_word_8(MEMORY_ADDRESS).await.unwrap());
    }

    #[pollster::test]
    async fn armv8a_read_word_aarch64_8() {
        const MEMORY_VALUE: u32 = 0xBA5EBA11;
        const MEMORY_ADDRESS: u64 = 0x12345679;
        const MEMORY_WORD_ADDRESS: u64 = 0x12345678;
