...
pub async fn open_device_from_selector(
    ...
    // Without CMSIS-DAP v1 there is no HID path to fall back to: hidapi is a C library and
    // WebUSB cannot reach HID devices, so wasm builds stop here.
    #[cfg(not(feature = "cmsisdap_v1"))]
    {
        let _ = hid_device_info;
