...
impl ESP32 {
    ...
    async fn disable_wdts(
        ...
        interface
            .write_word_32(TIMG0_WRITE_PROT, 0x50D83AA1)
            .await?; // write protection off
        interface.write_word_32(TIMG0_WDTCONFIG0, 0x0).await?;
        interface.write_word_32(TIMG0_WRITE_PROT, 0x0).await?; // write protection on
        ...
        interface
            .write_word_32(TIMG1_WRITE_PROT, 0x50D83AA1)
            .await?; // write protection off
        interface.write_word_32(TIMG1_WDTCONFIG0, 0x0).await?;
        interface.write_word_32(TIMG1_WRITE_PROT, 0x0).await?; // write protection on

        // rtc wdg
        const RTC_CNTL_BASE: u64 = 0x3ff48000;
        const RTC_WRITE_PROT: u64 = RTC_CNTL_BASE | 0xa4;
        const RTC_WDTCONFIG0: u64 = RTC_CNTL_BASE | 0x8c;
        interface.write_word_32(RTC_WRITE_PROT, 0x50D83AA1).await?; // write protection off
        interface.write_word_32(RTC_WDTCONFIG0, 0x0).await?;
        interface.write_word_32(RTC_WRITE_PROT, 0x0).await?; // write protection on
...
impl XtensaDebugSequence for ESP32 {
    ...
    async fn detect_flash_size(
        ...
        self.inner.detect_flash_size_esp32(session).await
    }
